        ...
        static void Main()
        {
            ...
            using (SQLiteConnection conn = new SQLiteConnection("Data Source=" + db_file))
            {
                ...
                using (SQLiteTransaction trans = conn.BeginTransaction())
                {
                    ...
                    cmd.CommandText = "INSERT INTO cname_tbl (racename, cname) VALUES (@racename, @cname)";
                    ...
                    foreach (var pair in table)
                    {
                        ...
                        // いったんデバッグで表示しているだけだが、このhtmlからレース結果（順位など）を正規表現で抜き出す。
                        //Debug.WriteLine(raceResultHtml);
                        //string htmlRaceResults = GetHtml(pair.Value);
                        //Debug.WriteLine(htmlRaceResults);
